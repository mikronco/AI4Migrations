...
class Shap():
    ...
    def fit(self): 
        if self.params["type"] == "classifier":
            f = lambda x: self.params["model"].predict_proba(x)[:,1]
        elif self.params["type"] == "regressor":
            f = lambda x: self.params["model"].predict(x)
        else:
            sys.exit("Specify model type: classifier or regressor!")
    ...
    def plot(self, expl, shap_values, plot_type, sample_id):
        if plot_type[0] == "local" and plot_type[1] == "waterfall":
            shp.plots.waterfall(shap_values[sample_id], show=False)
